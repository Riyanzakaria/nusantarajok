...
<body class="bg-white text-slate-900 antialiased min-h-screen flex flex-col selection:bg-accent-500/30 selection:text-accent-900" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)">
    <!-- Sleek Navbar V3 -->
    <header 
        class="fixed top-0 w-full z-50 transition-all duration-300 border-b"
        :class="scrolled ? 'bg-white/90 backdrop-blur-lg border-slate-200 py-3 shadow-sm' : 'bg-white border-transparent py-5'"
    >
        <nav class="max-w-7xl mx-auto px-6 flex items-center justify-between">
            <a href="{{ route('home') }}" class="font-display text-2xl font-black tracking-tighter text-slate-900 group flex items-center gap-2">
            ...
            <div class="flex items-center gap-8">
                <a href="{{ route('home') }}#layanan" class="text-sm font-semibold text-slate-600 hover:text-accent-500 transition-colors hidden md:block">Layanan</a>
                <a href="{{ route('home') }}#kalkulator" class="text-sm font-semibold text-slate-600 hover:text-accent-500 transition-colors hidden md:block">Estimasi</a>
                <a href="{{ route('tracker.index') }}" class="text-sm font-semibold text-slate-600 hover:text-accent-500 transition-colors hidden md:block">Lacak Progres</a>
                ...
                <div class="flex items-center gap-4 border-l border-slate-200 pl-6 ml-2">
                    <div class="flex flex-col items-end hidden sm:flex">
                        ...
                        <span class="text-xs font-medium text-slate-500 uppercase tracking-wider">{{ Auth::user()->role }}</span>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="touch-target rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 p-2.5 transition-colors" title="Keluar">
                ...
                @endauth
                @guest
                    <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-600 hover:text-accent-500 transition-colors">Login Admin</a>
                @endguest
            </div>
        </nav>
    </header>
    ...
    <!-- Minimalist Footer V3 -->
    <footer class="border-t border-slate-200 bg-slate-50 mt-auto">
        <div class="max-w-7xl mx-auto px-6 py-12 md:py-16">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12">
                <div class="col-span-1 md:col-span-2">
                    <a href="{{ route('home') }}" class="font-display text-2xl font-black tracking-tighter text-slate-900 mb-4 block">
                        AUTO-STITCH<span class="text-accent-500">.</span>
                    </a>
                ...
                <div>
                    <h4 class="font-bold text-slate-900 mb-4 uppercase tracking-wider text-sm">Navigasi</h4>
                    <ul class="space-y-3">
                        <li><a href="{{ route('home') }}#layanan" class="text-slate-500 hover:text-accent-500 transition-colors font-medium">Layanan Kami</a></li>
                        <li><a href="{{ route('home') }}#kalkulator" class="text-slate-500 hover:text-accent-500 transition-colors font-medium">Kalkulator Harga</a></li>
                        <li><a href="{{ route('tracker.index') }}" class="text-slate-500 hover:text-accent-500 transition-colors font-medium">Tracker Progres</a></li>
                    </ul>
                </div>
                <div>
                    <h4 class="font-bold text-slate-900 mb-4 uppercase tracking-wider text-sm">Kontak</h4>
                    <ul class="space-y-3">
                        <li class="text-slate-500 font-medium flex items-center gap-2">
            ...
            <div class="border-t border-slate-200 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="flex gap-4">
                    ...
                    <a href="#" class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-accent-500 hover:text-white transition-all duration-300">
                        IG
                    </a>
                    <a href="#" class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-accent-500 hover:text-white transition-all duration-300">
                        WA
